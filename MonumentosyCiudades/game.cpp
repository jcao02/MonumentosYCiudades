...
void Game::mainLoop() {
    ...
    /* Gets the video frame and displays it */
    if( (dataPtr = getAndDisplayVideo()) == NULL ) {
        arUtilSleep(2);
        return;
    }
...
int Game::checkCollisions( ObjectData_T object0, ObjectData_T object1, float collide_dist)
{
	float x1,y1,z1;
	float x2,y2,z2;
	float dist;
